...
contract('MarketplaceController app', ([root, authorized, unauthorized]) => {
  ...
  context('> #closeHatch', () => {
    beforeEach(async () => {
      await this.controller.openHatch({ from: authorized })
      await this.controller.contribute(HATCH_GOAL, { from: authorized })
    })
  ...
  context('> #contribute', () => {
    ...
    context('> sender has CONTRIBUTE_ROLE', () => {
      it('it should forward contribution', async () => {
        const receipt = await this.controller.contribute(HATCH_GOAL.div(bn(2)), { from: authorized })
    ...
    context('> sender does not have CONTRIBUTE_ROLE', () => {
      it('it should revert', async () => {
        await assertRevert(() => this.controller.contribute(HATCH_GOAL.div(bn(2)), { from: unauthorized }))
      })
    })
  })
  ...
  context('> #refund', () => {
    beforeEach(async () => {
      ...
      await this.controller.openHatch({ from: authorized })
      await this.controller.contribute(HATCH_GOAL.div(bn(2)), { from: authorized })
