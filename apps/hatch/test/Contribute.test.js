const { HATCH_STATE, HATCH_PERIOD, HATCH_GOAL, ZERO_ADDRESS } = require('@1hive/apps-marketplace-shared-test-helpers/constants')
    ...
    describe('When the sale has started', () => {
      ...
      describe('When a user buys project tokens', () => {
        ...
        describe('When the sale state is GoalReached', () => {
          ...
          it('A purchase cannot cause totalRaised to be greater than the hatchGoal', async () => {
            const raised = bn(await this.hatch.totalRaised())
            const remainingToFundingGoal = HATCH_GOAL.sub(raised)
            const userBalanceBeforePurchase = bn(await this.contributionToken.balanceOf(buyer2))

            const amount = HATCH_GOAL * 2
