const { HATCH_PERIOD, HATCH_STATE, HATCH_GOAL } = require('@1hive/apps-marketplace-shared-test-helpers/constants')
...
contract('Hatch, refund() functionality', ([anyone, appManager, buyer1, buyer2, buyer3, buyer4, buyer5]) => {
  const itAllowsBuyersToGetRefunded = startDate => {
    ...
    describe('When purchases have been made and the sale is ready to be closed', () => {
      before(async () => {
        this.hatch.mockSetTimestamp(startDate)
        await this.contributionToken.generateTokens(buyer4, HATCH_GOAL)
        await this.contributionToken.approve(this.hatch.address, HATCH_GOAL, { from: buyer4 })

        const totalRaised = (await this.hatch.totalRaised()).toNumber()
        await this.hatch.contribute(buyer4, HATCH_GOAL - totalRaised, { from: buyer4 })
      })
